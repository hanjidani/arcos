...
def calculate_mmd(X, Y, gamma=1.0):
    """
    Calculates the empirical MMD between two sets of samples X and Y.
    """
...
def run_mmd_experiment(n_samples=1000, alpha=1.0, n_test_samples=10000):
    """
    Runs a single experiment for the MMD-based bound.
    """
    ...
    # 3. Calculate MMD-based Bound
    mmd_dist = calculate_mmd(source_data, target_data)
    # Assuming the RKHS norm of the loss function is <= 1 for this test
    mmd_bound = g_q + g_q_tilde + D_Q + mmd_dist
...
if __name__ == '__main__':
    ...
    for alpha in alpha_values:
        print(f"Running MMD experiment for alpha = {alpha:.2f}...")
        results.append(run_mmd_experiment(alpha=alpha))
        
    df = pd.DataFrame(results)
    pd.set_option('display.width', 200)
    print("\n--- MMD-based Bound Results ---")
    print(df)
    
    df.to_csv('exp_3_mmd_bound/mmd_shift_results.csv', index=False)
    print("\nResults saved to exp_3_mmd_bound/mmd_shift_results.csv")
