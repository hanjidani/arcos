...
def run_mmd_covariance_experiment(n_samples=1000, beta=1.0, n_test_samples=10000):
    """
    Runs a single experiment for the MMD-based bound under covariance shift
    ...
    # Use median heuristic on the combined dataset to find a good gamma
...
if __name__ == '__main__':
    ...
    for beta in beta_values:
        print(f"Running MMD covariance experiment for beta = {beta:.2f}...")
        results.append(run_mmd_covariance_experiment(beta=beta))
        
    df = pd.DataFrame(results)
    pd.set_option('display.width', 200)
    print("\n--- MMD-based Bound (Covariance Shift with Optimal Gamma) ---")
    print(df)
    
    df.to_csv('exp_3_mmd_bound/mmd_covariance_shift_results.csv', index=False)
    print("\nResults saved to exp_3_mmd_bound/mmd_covariance_shift_results.csv")
